...
HardAVCEncoder::HardAVCEncoder(
    ...
    : SimpleHardOMXComponent(name, callbacks, appData, component),
      ...
      mDynamicEvent(OMX_FALSE),
      mStoredConfigDynamicResSetting(NULL),
      mDynamicEventChanged(false),
      mVideoMaxWidth(1920),
      mVideoMaxHeight(1080),
      mSliceGroup(NULL) {
...
HardAVCEncoder::~HardAVCEncoder() {
    ...
    CHECK(inQueue.empty());
    if (mStoredConfigDynamicResSetting) {
	delete mStoredConfigDynamicResSetting;
	mStoredConfigDynamicResSetting = NULL;
    }
}
...
OMX_ERRORTYPE HardAVCEncoder::initEncParams() {
    ...
    mParam.i_keyint_max=25;
    mParam.i_max_width = mVideoMaxWidth;
    mParam.i_max_height = mVideoMaxHeight;
...
OMX_ERRORTYPE HardAVCEncoder::internalGetParameter(
    ...
    switch (index) {
	//for dynamic reconfiguration
        case OMX_IndexINGConfigDynamicResSetting:
        case OMX_IndexINGParamDynamicResMaxSize:
	{
            return OMX_ErrorNone;
	}
...
OMX_ERRORTYPE HardAVCEncoder::internalSetParameter(
	...
	//for dynamic reconfiguration
        case OMX_IndexINGParamDynamicResMaxSize:
	{
	    OMX_VIDEO_PARAM_DYNAMIC_RES_MAXSIZE_TYPE *def = 
		(OMX_VIDEO_PARAM_DYNAMIC_RES_MAXSIZE_TYPE *) params;
	    int32_t mVideoMaxWidth = (def->nMaxWidth + 15) >> 4;
	    int32_t mVideoMaxHeight = (def->nMaxHeight + 15) >> 4;
	    OMX_PARAM_PORTDEFINITIONTYPE *pDef = &editPortInfo(0)->mDef;
	    pDef->nBufferSize = (mVideoMaxWidth * mVideoMaxHeight * 3) << 7 + 1024; //+1024 for extradata
	    ALOGV("internalSetParameter OMX_IndexINGParamDynamicResMaxSize pDef->nBufferSize=%d",pDef->nBufferSize);
	    return OMX_ErrorNone;
	}
	//for dynamic reconfiguration
        case OMX_IndexINGConfigDynamicResSetting:
	{
	    ALOGV("internalSetParameter OMX_IndexINGConfigDynamicResSetting");
	    OMX_VIDEO_CONFIG_DYNAMIC_RES_SETTING *def = 
		(OMX_VIDEO_CONFIG_DYNAMIC_RES_SETTING *) params;
	    if (mStoredConfigDynamicResSetting) {
		delete mStoredConfigDynamicResSetting;
		mStoredConfigDynamicResSetting = NULL;
	    }
	    mStoredConfigDynamicResSetting = new OMX_VIDEO_CONFIG_DYNAMIC_RES_SETTING;
	    memcpy(mStoredConfigDynamicResSetting, def, sizeof(OMX_VIDEO_CONFIG_DYNAMIC_RES_SETTING));
	    return OMX_ErrorNone;
	}
...
//#define WRITE_H264RAW_STREAM
#ifdef WRITE_H264RAW_STREAM
	FILE * rawh264_f = fopen("/data/rawh264", "a+");
#endif
	...
	mDynamicEventChanged = false;
	if (inHeader->nFlags & OMX_BUFFERFLAG_EXTRADATA) {
	    OMX_U8 *pTmp = inputData + inHeader->nFilledLen + 3;
	    OMX_OTHER_EXTRADATATYPE *def = (OMX_OTHER_EXTRADATATYPE *) (((OMX_U32) pTmp) & ~3);
	    while(def->eType != OMX_ExtraDataNone) {
		if (def->eType == OMX_ExtraDataINGDynamicRes) {
		    OMX_VIDEO_EXTRADATA_DYNAMIC_RES *res = (OMX_VIDEO_EXTRADATA_DYNAMIC_RES *) def->data;
		    if (mDynamicEvent != res->bDynamicEvent) {
			if (mStoredConfigDynamicResSetting) {
			    for (int i=0;i<mStoredConfigDynamicResSetting->nNumConfigParams;i++) {
				ALOGE("eConfigType = %d",mStoredConfigDynamicResSetting->eConfigType[i]);
				switch (mStoredConfigDynamicResSetting->eConfigType[i]) {
				    case OMX_IndexDynResFrameRate:
					mVideoFrameRate = mStoredConfigDynamicResSetting->nParam1[i] >> 16;
					mParam.i_fps_num = mVideoFrameRate;
					break;
				    case OMX_IndexDynResnPFrames:
					mParam.i_keyint_max = mStoredConfigDynamicResSetting->nParam1[i];
					break;
				    case OMX_IndexDynResBitRate:
					mVideoBitRate = mStoredConfigDynamicResSetting->nParam1[i];
					mParam.rc.i_fbr_bitrate = mVideoBitRate;
					break;
				    case OMX_IndexDynResProfileLevel:
					mAVCEncProfile = (AVCProfile)mStoredConfigDynamicResSetting->nParam1[i];
					mAVCEncLevel = (AVCLevel)mStoredConfigDynamicResSetting->nParam2[i];
					mParam.i_level_idc = mAVCEncLevel;
					break;
				    default:
					break;
				}
			...
			mVideoWidth = res->nWidth;
			mVideoHeight = res->nHeight;
			mParam.i_width = mVideoWidth;
			mParam.i_height = mVideoHeight;
			mDynamicEvent = res->bDynamicEvent;
			updatePortDefinitions();
			mDynamicEventChanged = true;
			mSpsPpsHeaderReceived = false;
			mNumInputFrames = -1;
			mParam.need_sps_init = 1;
			x264_encoder_reconfig(h,&mParam);
			ALOGE("get reconfiguration info and apply it.(%d x %d)",mVideoWidth,mVideoHeight);
		    }
		}
		def = (OMX_OTHER_EXTRADATATYPE *) (((OMX_U8 *) def) + def->nSize);
	    }
	}
	if (mDynamicEventChanged) {
#ifdef WRITE_H264RAW_STREAM
	...
	for(i=0;i<i_nal;i++){
	  ...
	  case 0x1://P nal
	    ...
	    memcpy(outPtr+dataLength, nal[i].p_payload, size);
	    dataLength += size;	    
	    break;
	  }
	}
...
void HardAVCEncoder::updatePortDefinitions() {
